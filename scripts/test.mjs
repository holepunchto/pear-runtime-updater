...
await runLifecycleScripts(fixtureDir, readAllowlist(fixtureDir))
...
function readAllowlist(dir) {
  ...
  const raw = pkg.allowScripts
  if (raw === undefined || raw === null) return []
  if (Array.isArray(raw)) return raw
  if (typeof raw === 'object') return raw[platform] || []
  throw new Error('allowScripts must be an array or a platform map')
}
